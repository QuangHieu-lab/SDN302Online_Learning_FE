...
const InstructorProgress = () => {
    ...
    const [selectedStudentEntry, setSelectedStudentEntry] = useState(null);
    ...
    const openStudentProgress = (entry) => {
        setSelectedStudentEntry(entry);
        onOpen();
    };

    const closeStudentProgress = () => {
        setSelectedStudentEntry(null);
        onClose();
    };
    ...
    return (
        <Box bg={bgColor} minH="100vh">
                        ...
                        {loadingCourses || loadingProgress ? (
                            ...
                            <VStack align="stretch" spacing={4}>
                                ...
                                <Box bg={cardBg} border="1px" borderColor={borderColor} borderRadius="xl" p={5}>
                                    ...
                                    <Text fontSize="sm" color={mutedColor} mb={4}>
                                        Click each learner to view detailed module and lesson progress.
                                    </Text>
                                            ...
                                            >
                                                <VStack spacing={4} align="stretch">
                                                    <HStack justify="space-between" align="start">
                                                        ...
                                                        <Badge colorScheme={getProgressColor(Number(entry.progressPercent || 0))}>
                                                            {Number(entry.progressPercent || 0) >= 100 ? "Completed" : "In Progress"}
                                                        </Badge>
                                                    </HStack>

                                                    <Flex justify="center">
                                                        <CircularProgress
                                                            value={Number(entry.progressPercent || 0)}
                                                            color={`${getProgressColor(Number(entry.progressPercent || 0))}.400`}
                                                            trackColor={chartTrackBg}
                                                            thickness="10px"
                                                            size="120px"
                                                        >
                                                            <CircularProgressLabel fontWeight="bold" color={textColor}>
                                                                {entry.progressPercent}%
                                                            </CircularProgressLabel>
                                                        </CircularProgress>
                                                    </Flex>
                                                    ...
                                                    <Box textAlign="center">
                                                        <Text fontSize="sm" color={mutedColor}>
                                                            {entry.completedLessons}/{entry.totalLessons} lessons completed
                                                        </Text>
                                                    </Box>
                                                    ...
                                                    <Button
                                                        ...
                                                        colorScheme="blue"
                                                        variant="outline"
                                                        onClick={() => openStudentProgress(entry)}
                                                    >
                                                        View Details
                                                    </Button>
                                                </VStack>
            ...
            <Modal isOpen={isOpen} onClose={closeStudentProgress} size="5xl" scrollBehavior="inside">
                <ModalOverlay />
                <ModalContent>
                    <ModalHeader>
                        {selectedStudentEntry?.student?.fullName || "Learner Progress"}
                    </ModalHeader>
                    <ModalCloseButton />
                    <ModalBody pb={6}>
                                ...
                                <SimpleGrid columns={{ base: 1, lg: 4 }} spacing={4}>
                                    <Box>
                                        ...
                                        <Text fontWeight="medium" color={textColor}>
                                            {selectedStudentEntry.student?.email}
                                        </Text>
                                    </Box>
                                    <Box>
                                        <Text fontSize="sm" color={mutedColor}>Progress</Text>
                                        <Text fontWeight="bold" color={textColor}>
                                            {selectedStudentEntry.progressPercent}% ({selectedStudentEntry.completedLessons}/{selectedStudentEntry.totalLessons} lessons)
                                        </Text>
                                    </Box>
                                    <Box>
                                        <Text fontSize="sm" color={mutedColor}>Current Module</Text>
                                        <Text fontWeight="medium" color={textColor}>
                                            {selectedStudentEntry.currentModule?.moduleTitle || "Completed all modules"}
                                        </Text>
                                    </Box>
                                    <Box>
                                        <Text fontSize="sm" color={mutedColor}>Current Lesson</Text>
                                        <Text fontWeight="medium" color={textColor}>
                                            {selectedStudentEntry.currentLesson?.lessonTitle || "Completed all lessons"}
                                        </Text>
                                    </Box>
                                </SimpleGrid>

                                <Accordion allowMultiple>
                                    {selectedStudentEntry.modules.map((module) => (
                                                    ...
                                                    {module.lessons.map((lesson) => (
                                                        ...
                                                        >
                                                            <HStack justify="space-between" align="start" mb={2}>
                                                                ...
                                                                <HStack spacing={2}>
                                                                    {selectedStudentEntry.currentLesson?.lessonId === lesson.lessonId && (
