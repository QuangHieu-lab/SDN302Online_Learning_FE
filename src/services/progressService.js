...
/** Mirror currentSecond into lastWatchedSecond for backends that read either field. */
const withVideoPayload = (payload = {}) => {
  const p = { ...payload };
  if (p.currentSecond != null && p.lastWatchedSecond == null) {
    p.lastWatchedSecond = p.currentSecond;
  }
  return p;
};
...
export const progressAPI = {
  ...
  updateLessonVideoProgress: (lessonId, payload) =>
    fetchWithAuth(`${API_URL}/progress/lessons/${lessonId}/video`, {
      method: "POST",
      body: JSON.stringify(withVideoPayload(payload)),
    }),
